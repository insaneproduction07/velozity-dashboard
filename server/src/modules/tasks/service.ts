import { PrismaClient, Role, TaskStatus, TaskPriority, ActivityType, NotificationType, Prisma } from '@prisma/client';
...
async function checkTaskAccess(userId: string, userRole: string, taskId: string): Promise<{ allowed: boolean; task?: TaskWithRelations }> {
...
export async function getTasks(
  ...
  query: { page: number; limit: number; status?: TaskStatus; assigneeId?: string }
): Promise<{ items: ReturnType<typeof toTaskResponse>[]; page: number; limit: number; total: number }> {
...
export async function createTask(
  projectId: string,
  data: { title: string; description?: string | null; priority: TaskPriority; assigneeId?: string | null; dueDate?: Date | null },
  requestingUser: { userId: string; role: string }
  ...
  emitTaskCreated(projectId, toTaskResponse(task as TaskWithRelations));
  emitActivityNew(projectId, { type: ActivityType.TASK_CREATED, message: `Task "${toTaskResponse(task as TaskWithRelations).title}" created`, userId: requestingUser.userId, projectId, taskId: task.id, createdAt: new Date() });

  return toTaskResponse(task as TaskWithRelations);
}

export async function getTaskById(taskId: string, requestingUser: { userId: string; role: string }): Promise<ReturnType<typeof toTaskResponse>> {
...
export async function updateTask(
  taskId: string,
  data: { title?: string; description?: string | null; status?: TaskStatus; priority?: TaskPriority; dueDate?: Date | null },
  requestingUser: { userId: string; role: string }
): Promise<ReturnType<typeof toTaskResponse>> {
  const { allowed, task } = await checkTaskAccess(requestingUser.userId, requestingUser.role, taskId);
  if (!allowed || !task) {
  ...
  if (data.status && data.status !== task.status) {
    ...
    emitTaskUpdated(task.projectId, toTaskResponse(updatedTask as TaskWithRelations));
    ...
    return toTaskResponse(updatedTask as TaskWithRelations);
  }
...
export async function assignTask(
  ...
  emitTaskAssigned(task.projectId, toTaskResponse(updatedTask as TaskWithRelations), { id: assignee.id, name: assignee.name, email: assignee.email, role: assignee.role, avatarUrl: assignee.avatarUrl });
  emitActivityNew(task.projectId, { type: ActivityType.TASK_ASSIGNED, message: `Task "${toTaskResponse(updatedTask as TaskWithRelations).title}" assigned to ${assignee.name}`, userId: requestingUser.userId, projectId: task.projectId, taskId, createdAt: new Date() });

  return toTaskResponse(updatedTask as TaskWithRelations);
}

export async function deleteTask(taskId: string, requestingUser: { userId: string; role: string }): Promise<void> {
  const { allowed, task } = await checkTaskAccess(requestingUser.userId, requestingUser.role, taskId);
  if (!allowed || !task) {

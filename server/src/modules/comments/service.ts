import { PrismaClient, ActivityType, TaskStatus, Prisma } from '@prisma/client';
...
async function checkTaskAccess(userId: string, userRole: string, taskId: string): Promise<{ allowed: boolean; task?: { id: string; projectId: string; title: string } }> {
...
): Promise<{ items: ReturnType<typeof toCommentResponse>[]; page: number; limit: number; total: number }> {
